...
def test_build_updated_minicourse(minicourse_get_item_by_id: Mock) -> None:
    ...
    MOCK_NEW_ID = "PREFIX2"
    MOCK_NEW_NAME = "MIN2"
    MOCK_NEW_THUMB_EXT = "JPG"

    updated_minicourse = subject.build_updated_minicourse(
        MOCK_MINICOURSE_ID,
        id=MOCK_NEW_ID,
        name=MOCK_NEW_NAME,
        thumb_ext=MOCK_NEW_THUMB_EXT,
    )

    assert not (updated_minicourse.name == mock_minicourse.name)
    assert updated_minicourse.name == MOCK_NEW_NAME
    assert not (updated_minicourse.thumb_ext == mock_minicourse.thumb_ext)
    assert updated_minicourse.thumb_ext == MOCK_NEW_THUMB_EXT
